...
const char* elf_path_forwarder = "sdmc:/retroarch/forwarders/test.3dsx";
      ...
      void *args, void *params_data)
{
   ...
   FILE* path_fp = fopen("romfs:/path.txt", "r");
   if (!path_fp) {
         RARCH_LOG("File open error!\n");
   }
   else
   {  
      RARCH_LOG("did fopen");
      fgets(uniqueID, sizeof(uniqueID), path_fp);


      char forwarderPath[PATH_MAX_LENGTH];
      fill_pathname_basedir(forwarderPath, elf_path_forwarder, sizeof(forwarderPath));
      // forwarderPath = sdmc:/retroarch/forwarders

      fill_pathname_join(g_defaults.dir.core_info, forwarderPath,
         uniqueID, sizeof(g_defaults.dir.core_info));


      fill_pathname_join(forwarderPath, forwarderPath, uniqueID, sizeof(forwarderPath));
      // forwarderPath = sdmc:/retroarch/forwarders/uniqueID

      //path_mkdir(uniqueID);
      //RARCH_LOG("did mkdir");


      fill_pathname_join(g_defaults.dir.savestate, forwarderPath,
         "savestates", sizeof(g_defaults.dir.savestate));
      fill_pathname_join(g_defaults.dir.sram, forwarderPath,
...
static void frontend_ctr_deinit(void *data)
{
   ...
   *verbose           = true;
   ...
   // Asdolo: Removed 'Press Start' when exit
   // if(gfxBottomFramebuffers[0] == (u8*)currentConsole->frameBuffer) wait_for_input();
...
/*
   // Asdolo 1.1: restore state correctly when suspending/exiting app.
...
#endif
   gfxInit(GSP_BGR8_OES,GSP_BGR8_OES,false);   // Asdolo: Changed bottom screen format tp BGR8
   gfxSet3D(false);
   
   // Asdolo: removed console
