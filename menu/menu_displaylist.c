      ...
      menu_displaylist_info_t *info)
{
   ...
   if (!rarch_ctl(RARCH_CTL_IS_DUMMY_CORE, NULL))
   {
      ...
      // Sakitoshi: declare control to show/hide the disk options menu
      struct retro_disk_control_callback *control = NULL;

      runloop_ctl(RUNLOOP_CTL_SYSTEM_INFO_GET, &system);
      // Sakitoshi: retrieve control value
      ...
      */
      // Asdolo: Removed 'Take Screenshot' menu option from Quick Menu
      /*
      menu_entries_push(info->list,
      ...
      */
      ...
      // Asdolo: Removed 'Save slot' menu option from Quick Menu
      /*
      ...
      */
      ...
      // Asdolo: Removed 'Core Options' menu option from Quick Menu
      /*
      menu_entries_push(info->list,
      ...
      */
      ...
      if ((!rarch_ctl(RARCH_CTL_IS_DUMMY_CORE, NULL)) && system && control->get_num_images() > 1)

         // Asdolo: Removed 'Core Disk Options' menu option from Quick Menu
         // Sakitoshi: Enable it again for PS1. but only if there is more than 1 disc.
      ...
      menu_displaylist_info_t *info)
{
   ...
   // Asdolo: Removed 'Cheat File Save As' menu option from Core Cheat Options
   /*
   menu_entries_push(info->list,
      ...
      menu_displaylist_info_t *info)
{
   ...
   for (p = 0; p < 1; p++)
   {
      ...
      menu_entries_push(info->list,
            core_option_get_desc(system->core_options, p + 4), "",
            MENU_SETTINGS_CORE_OPTION_START + p + 4, 0, 0);
      */
   ...
   for (p = 0; p < 1; p++)
   {
      for (retro_id = 0; retro_id < RARCH_FIRST_CUSTOM_BIND + 4; retro_id++)
      {
         ...
         snprintf(desc_label, sizeof(desc_label),
               "PS1 %s : ", description); // Sakitoshi: simplify label
...
bool menu_displaylist_ctl(enum menu_displaylist_ctl_state type, void *data)
{
   ...
   switch (type)
   {
      ...
      case DISPLAYLIST_OPTIONS_DISK:
         ...
         /* Sakitoshi: Disallow appending more discs
         menu_entries_push(info->list,
