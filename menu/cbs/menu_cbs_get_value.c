...
// Sakitoshi: new function to inform if the tray is open or closed
static void menu_action_setting_disp_set_label_menu_disk_cycle_tray_status(
      file_list_t* list,
      unsigned *w, unsigned type, unsigned i,
      const char *label,
      char *s, size_t len,
      ...
      char *s2, size_t len2)
{
   ...
   *s = '\0';
   ...
   status = control->get_eject_state();

   if (status == ejected)
      strlcpy(s, menu_hash_to_str(MENU_LABEL_VALUE_OPEN_ARCHIVE), len);
   else
      strlcpy(s, menu_hash_to_str(MENU_LABEL_VALUE_LOAD_ARCHIVE), len);
}
   ...
   else
   {
      switch (type)
      {
         ...
         case MENU_SETTINGS_CORE_DISK_OPTIONS_DISK_INDEX:
            ...
            break;
         // Sakitoshi: show in the menu if the tray is open or closed
         case MENU_SETTINGS_CORE_DISK_OPTIONS_DISK_CYCLE_TRAY_STATUS:
            BIND_ACTION_GET_VALUE(cbs,
               menu_action_setting_disp_set_label_menu_disk_cycle_tray_status);
            break;
         default:
