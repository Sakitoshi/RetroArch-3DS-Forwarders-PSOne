...
static void config_set_defaults(void)
{
   ...
   *settings->menu_config_directory = '\0';
   ...
   // Asdolo: set RetroArch language based on system language
   u8 language_3ds = 0;
   Result res;

   unsigned int language_ra = 0;
   ...
   switch (language_3ds)
   {
      ...
      case CFG_LANGUAGE_PT:
         language_ra = RETRO_LANGUAGE_PORTUGUESE;
         break;
   ...
   settings->user_language = language_ra;
...
static bool config_load_file(const char *path, bool set_defaults)
{
         ...
         config_get_path(conf, "savefile_directory", tmp_str, sizeof(tmp_str)))
   {
      ...
      {  
         path_mkdir(global->dir.savefile);               // Asdolo
         ...
         config_get_path(conf, "savestate_directory", tmp_str, sizeof(tmp_str)))
   {
      ...
      {
         path_mkdir(global->dir.savestate);              // Asdolo
...
static void config_load_core_specific(void)
{
   ...
   *global->path.core_specific_config = '\0';
   ...
   if (settings->core_specific_config)
   {
      ...
      if (!config_load_file("romfs:/retroarch.cfg", true))     // Asdolo
...
bool config_load_override(void)
{
   ...
   if (!config_load_file("romfs:/retroarch.cfg", false))          // Asdolo
      return false;
...
{
   global_t *global = global_get_ptr();
   bool ret = config_load_file("romfs:/retroarch.cfg", false);          // Asdolo
