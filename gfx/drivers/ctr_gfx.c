...
static void ctr_lcd_aptHook(APT_HookType hook, void* param)
{
   ...
   if (hook == APTHOOK_ONRESTORE || hook == APTHOOK_ONWAKEUP)
   {
      ...
      if (not_2DS && srvGetServiceHandle(&lcd_handle, "gsp::Lcd") >= 0)
      {
         ...
         cmdbuf[0] = (bottom_screen_buffer_size == 0)? 0x00120040: 0x00110040;
         cmdbuf[1] = 2;
   ...
   if (hook == APTHOOK_ONSUSPEND)
   {
      ...
      */
      ...
      //GSPGPU_FlushDataCache(gfxGetFramebuffer(GFX_TOP, GFX_RIGHT, NULL, NULL), 400 * 240 * 3);
      GSPGPU_FlushDataCache(gfxGetFramebuffer(GFX_TOP, GFX_LEFT, NULL, NULL), 400 * 240 * 3);
      ...
      const input_driver_t** input, void** input_data)
{
   ...
   // Asdolo: load bottom screen image if there is any. Otherwise, the bottom screen will shut down
   //u8 language = 0;
   //Result res;
   ...
   char bot_img_path[22];

   //sprintf(bot_img_path, "romfs:/bottom_%u.bin", language);
   ...
   if (file)
   {
      ...
      //allocate a buffer
      bottom_screen_buffer = malloc(bottom_screen_buffer_size);
      ...
      unsigned pitch, const char* msg)
{
   ...
   // Asdolo: Removed the debug console toggle
   /*
   state_tmp = hidKeysDown();
   hidTouchRead(&state_tmp_touch);
   
   if((state_tmp & KEY_TOUCH) && (state_tmp_touch.py < 240))
   {
      Handle lcd_handle;
      u8 not_2DS;
      extern PrintConsole* currentConsole;
   ...
   */
   ...
   if (bottom_screen_counter == 2)
   {
      memcpy(gfxGetFramebuffer(GFX_BOTTOM, GFX_LEFT, NULL, NULL), bottom_screen_buffer, bottom_screen_buffer_size);
      bottom_screen_counter = 0;;
   }
   else
   {
      bottom_screen_counter++;
   }
